...
BOTTOM_OUTER_THICKNESS = SUPPORT_THICKNESS_OFFSET
...
def create_bottom_box_mesh() -> trimesh.Trimesh:
    box_mesh = trimesh.creation.box((OUTER_WIDTH, OUTER_HEIGHT, BOTTOM_OUTER_THICKNESS))

    diff_mesh = trimesh.creation.box((OUTER_WIDTH-THICKNESS, OUTER_HEIGHT-THICKNESS, BOTTOM_OUTER_THICKNESS))
    diff_mesh.apply_translation([0, 0, BOTTOM_OUTER_THICKNESS/2-0.25])
    box_mesh = box_mesh.difference(diff_mesh)
